...
def get_tests(config={}):
    tests = []
    from Cryptodome.SelfTest import Cipher

    tests += Cipher.get_tests(config=config)
    from Cryptodome.SelfTest import Hash

    tests += Hash.get_tests(config=config)
    from Cryptodome.SelfTest import Protocol

    tests += Protocol.get_tests(config=config)
    from Cryptodome.SelfTest import PublicKey

    tests += PublicKey.get_tests(config=config)
    from Cryptodome.SelfTest import Random

    tests += Random.get_tests(config=config)
    from Cryptodome.SelfTest import Util

    tests += Util.get_tests(config=config)
    from Cryptodome.SelfTest import Signature

    tests += Signature.get_tests(config=config)
    from Cryptodome.SelfTest import IO

    tests += IO.get_tests(config=config)
    from Cryptodome.SelfTest import Math
